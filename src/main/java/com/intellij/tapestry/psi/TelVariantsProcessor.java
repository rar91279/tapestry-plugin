...
abstract class TelVariantsProcessor<T> implements PsiScopeProcessor {
  ...
  public boolean execute(@NotNull final PsiElement element, @NotNull final ResolveState state) {
    ...
    if (isMethod) {
      ...
      if (!myMethodCall &&
        ...
        myPropertyAccessors.addMethod(method, state.<PsiSubstitutor>get(PsiSubstitutor.KEY), false);
      }
      if (myForCompletion || myMethodCall && myReferenceName.equalsIgnoreCase(namedElement.getName())) {
        if (myMethods != null) myMethods.addMethod((PsiMethod)namedElement, state.<PsiSubstitutor>get(PsiSubstitutor.KEY), false);
      }
    }
    else if (isField) {
      ...
      if (myForCompletion || myMethodCall && myReferenceName.equalsIgnoreCase(getterName)) {
        myMethods.addMethod(new TapestryAccessorMethod(field, true, getterName), state.<PsiSubstitutor>get(PsiSubstitutor.KEY), false);
      ...
      if (!field.hasModifierProperty(FINAL) && (myForCompletion || myMethodCall && myReferenceName.equalsIgnoreCase(setterName))) {
        myMethods.addMethod(new TapestryAccessorMethod(field, false, setterName), state.<PsiSubstitutor>get(PsiSubstitutor.KEY), false);
  ...
  public T @NotNull [] getVariants(T[] array) {
    ...
    return myResult.<T>toArray(array);
  }
}
