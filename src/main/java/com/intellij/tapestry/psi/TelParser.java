...
public class TelParser implements PsiParser {
  private static final Key<String> LAST_FOUND_IDENT = Key.<String>create("LAST_FOUND_IDENT");

  @Override
  @NotNull
  public ASTNode parse(IElementType root, PsiBuilder builder) {
    //builder.setDebugMode(true);
    final ASTNode contextNode = builder.<ASTNode>getUserData(TAP5_CONTEXT_NODE_KEY);
    ...
    if (consumeToken(builder, TAP5_EL_START)) {
      ...
      if (referenceExpression != null) {
        if(consumeOptionalToken(builder, TAP5_EL_COLON)) {
          try {
            if("prop".equalsIgnoreCase(builder.<String>getUserData(LAST_FOUND_IDENT))) {
            ...
            builder.<String>putUserData(LAST_FOUND_IDENT, null);
          }
        } else {
  ...
  private static PsiBuilder.Marker tryToConsumeIdentifierAndMark(PsiBuilder builder) {
    ...
    PsiBuilder.Marker mark = builder.mark();
    builder.<String>putUserData(LAST_FOUND_IDENT, builder.getTokenText());
    builder.advanceLexer();
    return mark;
  }
