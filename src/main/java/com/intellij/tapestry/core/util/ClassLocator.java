...
public class ClassLocator {
    ...
    private void loadJar(URL resource) throws IOException {
        ...
        String spec = resource.getFile(); // e.g. file:/path/plugin.jar!/documentation/core/components
        int separator = spec.indexOf("!/");
        String jarSpec = separator == -1 ? spec : spec.substring(0, separator);
        File jarPath;
        try {
            jarPath = new File(new URL(jarSpec).toURI());
        } catch (URISyntaxException ex) {
            throw new IOException("Malformed jar URL: " + resource, ex);
        }
