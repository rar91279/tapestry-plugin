...
public class TapestryProjectViewPane extends AbstractProjectViewPane implements FileSystemListener, TapestryModelChangeListener {
  ...
  private StructureTreeModel<TapestryProjectTreeStructure> myStructureTreeModel;
  ...
  public void addToolbarActions(@NotNull DefaultActionGroup defaultactiongroup) {
    for (AnAction action : defaultactiongroup.getChildActionsOrStubs()) {
  ...
  public ActionCallback updateFromRoot(boolean b) {
    if (myStructureTreeModel != null) myStructureTreeModel.invalidateAsync();
  ...
  public Object getData(@NotNull String dataId) {
    ...
    if (LangDataKeys.IDE_VIEW.is(dataId)) {
      SimpleNode selected = getSelectedSimpleNode();
      if (selected == null) {
        return null;
      }

      Object element = selected.getElement();

      if (!(element instanceof PsiDirectory) && !(element instanceof PsiFile) ||
          IdeaUtils.<ExternalLibraryNode>findFirstParent(getSelectedTreeNode(), ExternalLibraryNode.class) != null) {
        return null;
      }
      return myIdeView;
    }
    ...
    if (PlatformCoreDataKeys.MODULE.is(dataId)) {
      SimpleNode selected = getSelectedSimpleNode();
      if (selected instanceof TapestryNode) {
        return ((TapestryNode)selected).getModule();
      }
    }

    if (CommonDataKeys.NAVIGATABLE.is(dataId)) {
      SimpleNode selected = getSelectedSimpleNode();
      if (selected != null && selected.getElement() instanceof PresentationLibraryElement) {
        return ((IntellijResource)((PresentationLibraryElement)selected.getElement()).getElementClass().getFile())
            .getPsiFile();
      }
    }
  ...
  private DefaultMutableTreeNode getSelectedTreeNode() {
    TreePath path = myTree == null ? null : myTree.getSelectionPath();
    Object component = path == null ? null : path.getLastPathComponent();
    return component instanceof DefaultMutableTreeNode ? (DefaultMutableTreeNode)component : null;
  }

  private SimpleNode getSelectedSimpleNode() {
    DefaultMutableTreeNode node = getSelectedTreeNode();
    Object userObject = node == null ? null : node.getUserObject();
    return userObject instanceof SimpleNode ? (SimpleNode)userObject : null;
  }

  Object getSelectedNodeElement() {
    SimpleNode selected = getSelectedSimpleNode();
    return selected == null ? null : selected.getElement();
  }
  ...
  private void initTree() {
    TapestryProjectTreeStructure treeStructure = new TapestryProjectTreeStructure(new RootNode(myProject));
    ...
    myTree = new ProjectViewTree(asyncTreeModel) {
          ...
          if (toolWindow != null) {
            ...
            if (selectedNode instanceof PageNode || selectedNode instanceof ComponentNode || selectedNode instanceof MixinNode) {
              toolWindow.update((Module)getData(PlatformCoreDataKeys.MODULE.getName()), selectedNode.getElement(),
                                Collections.<IJavaClassType>singletonList(((PresentationLibraryElement)selectedNode.getElement()).getElementClass()));
            }
            ...
            if (selectedNode instanceof ClassNode || selectedNode instanceof FileNode) {
              ...
              if (parentSelectedNode.getElement() instanceof PresentationLibraryElement) {
                toolWindow.update((Module)getData(PlatformCoreDataKeys.MODULE.getName()), parentSelectedNode.getElement(),
                                  Collections
                                    .<IJavaClassType>singletonList(((PresentationLibraryElement)parentSelectedNode.getElement()).getElementClass()));
              }
              else {
                ...
                if (component != null) {
                  toolWindow.update((Module)getData(PlatformCoreDataKeys.MODULE.getName()), component,
                                    Collections.<IJavaClassType>singletonList(component.getElementClass()));
                }
              }
            }
