...
public class TagDocumentationNavigation extends AnAction {
  ...
  public void actionPerformed(@NotNull AnActionEvent event) {
    ...
    Project project = event.<Project>getData(CommonDataKeys.PROJECT);
    if (project == null) return;
    Module module = event.<Module>getData(PlatformCoreDataKeys.MODULE);
    ...
    metatoolWindow.update(module, component, Collections.<com.intellij.tapestry.core.java.IJavaClassType>singletonList(component.getElementClass()));
  }

  @Nullable
  private static TapestryComponent getTapestryComponent(AnActionEvent event) {
    Editor editor = event.<Editor>getData(CommonDataKeys.EDITOR);
    PsiFile psiFile = event.<PsiFile>getData(CommonDataKeys.PSI_FILE);

    if (editor == null || psiFile == null) return null;

    int caretOffset = editor.getCaretModel().getOffset();
    XmlTag tag = PsiTreeUtil.<XmlTag>getParentOfType(psiFile.findElementAt(caretOffset), XmlTag.class);
