...
public abstract class AddNewElementAction<T extends PackageNode> extends AnAction {
  ...
  public void update(@NotNull AnActionEvent event) {
    ...
    Object data = event.<Object>getData(SELECTED_ITEM);
    ...
    if (element == null) {
      PsiElement eventPsiElement = event.<PsiElement>getData(CommonDataKeys.PSI_ELEMENT);
    ...
    // it's the Tapestry view | folder
      ...
      if (session.<Boolean>compute(this, "findParent", ActionUpdateThread.EDT, () ->
        (IdeaUtils.<T>findFirstParent(element, nodeClass) != null || nodeClass.isInstance(element.getUserObject())) &&
        IdeaUtils.<LibrariesNode>findFirstParent(element, LibrariesNode.class) == null)) {
        enabled = true;
      }
    }
