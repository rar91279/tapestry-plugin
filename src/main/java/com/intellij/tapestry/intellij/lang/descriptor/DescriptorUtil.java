...
final class DescriptorUtil {
  ...
  public static XmlAttributeDescriptor[] getAttributeDescriptors(@NotNull XmlTag context) {
    ...
    Collections.<XmlAttributeDescriptor>addAll(listResult, result);
    for (Mixin mixin : mixins) {
      ContainerUtil.<XmlAttributeDescriptor, List<XmlAttributeDescriptor>>addAll(listResult, getAttributeDescriptors(mixin, null));
    }
    return listResult.<XmlAttributeDescriptor>toArray(XmlAttributeDescriptor.EMPTY);
  }
  ...
  public static XmlElementDescriptor[] getTmlSubelementDescriptors(@NotNull XmlTag context, TapestryNamespaceDescriptor descriptor) {
    ...
    return ArrayUtil.<XmlElementDescriptor>mergeArrays(namespaceElements, parameterElements);
  }
  ...
  private static XmlElementDescriptor[] getElementDescriptors(@NotNull Collection<? extends PresentationLibraryElement> elements,
    ...
    final XmlElementDescriptor[] descriptorsFromSchema = descriptor.getSuperRootElementsDescriptors(PsiTreeUtil.<XmlDocument>getParentOfType(context, XmlDocument.class));
    return ArrayUtil.<XmlElementDescriptor>mergeArrays(descriptors, descriptorsFromSchema);
  }
  ...
  private static XmlElementDescriptor[] getParameterDescriptors(@NotNull final TapestryComponent component,
    ...
    final List<XmlElementDescriptor> result =
      new ArrayList<>(ContainerUtil.<TapestryParameter, XmlElementDescriptor>map(component.getParameters().values(), mapping));
    for (Mixin mixin : mixins) {
      result.addAll(ContainerUtil.<TapestryParameter, XmlElementDescriptor>map(mixin.getParameters().values(), mapping));
    }
    return result.<XmlElementDescriptor>toArray(XmlElementDescriptor.EMPTY_ARRAY);
  }
  ...
  private static TmlFile getTmlFile(XmlTag tag) {
    ...
    XmlElement parentTag = tag.<XmlElement>getUserData(XmlElement.INCLUDING_ELEMENT);
  ...
  private static List<Mixin> findMixins(@Nullable XmlTag tag) {
    if (tag == null) {
      return Collections.<Mixin>emptyList();
    }
